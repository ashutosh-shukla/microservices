...
public class KYCUIController {
    ...
    // Handle file upload - calls backend service through API Gateway
    @PostMapping("/upload")
    public String uploadKYCDocuments(
        ...
        try {
            ...
            ResponseEntity<Map> response = restTemplate.postForEntity(
                API_GATEWAY_URL + "/kyc/api/upload", 
                requestEntity, 
                Map.class
            );
    ...
    // Admin panel - fetch all documents through API Gateway
    @GetMapping("/admin")
    public String showAdmin(Model model) {
        try {
            ResponseEntity<List> response = restTemplate.getForEntity(
                API_GATEWAY_URL + "/kyc/api/admin", List.class);
    ...
    // View specific document through API Gateway
    @GetMapping("/admin/view/{id}")
    public String viewDocument(@PathVariable Long id, Model model) {
        try {
            ResponseEntity<Map> response = restTemplate.getForEntity(
                API_GATEWAY_URL + "/kyc/api/admin/view/" + id, Map.class);
    ...
    // Delete document through API Gateway
    @PostMapping("/admin/delete/{id}")
    public String deleteDocument(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        try {
            restTemplate.postForObject(API_GATEWAY_URL + "/kyc/api/admin/delete/" + id, null, String.class);
    ...
    // Update document status through API Gateway
    ...
    public String healthCheck() {
        try {
            String response = restTemplate.getForObject(API_GATEWAY_URL + "/health/kyc", String.class);
